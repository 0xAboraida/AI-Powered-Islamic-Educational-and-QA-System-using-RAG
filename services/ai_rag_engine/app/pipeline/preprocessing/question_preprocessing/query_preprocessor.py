...
# Look for the 'services' directory in the parent path
for parent in [current_path] + list(current_path.parents):
    if (parent / "services").is_dir() and (
        parent / "services" / "ai_rag_engine"
    ).is_dir():
        project_root = parent
        break
...
from services.ai_rag_engine.app.pipeline.preprocessing.question_preprocessing.prompt import (
            ...
            try:
                ...
                if "gemini" in self.actual_model_name.lower():
                    ...
                    base_url = (
                        ...
                        if api_key.startswith("github_pat")
                        else None
                    )
                    llm = ChatOpenAI(
                        model=self.actual_model_name,
