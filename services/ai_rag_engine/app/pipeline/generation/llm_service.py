...
class LLMService:
    ...
    async def generate_response(
        ...
        def estimate_tokens(text: str) -> int:
            # Arabic text usually consumes more tokens (around 2 to 3 tokens per word)
            return max(len(text) // 4, int(len(text.split()) * 2.5))
        ...
        for attempt, key in enumerate(all_keys):
            try:
                ...
                success = True
                logger.info(f"[LLMService] Response successfully generated by Primary LLM (Gemini, Key #{attempt+1})")
        ...
        if not success:
            ...
            try:
                ...
                full_generated_text = "\n\n*(عذراً، الخادم الأساسي مشغول. تم توليد الإجابة بالنموذج الإحتياطي)*\n\n" + ai_message_content
                current_llm_model = fallback_model
                logger.info(f"[LLMService] Response successfully generated by Fallback LLM ({provider_str} - {settings.FALLBACK_MODEL_NAME})")
                    
            except Exception as fallback_e:
                logger.error(f"[LLMService] Both Primary and Fallback LLMs failed! Error: {fallback_e}", exc_info=True)
                return {
                    "answer": "عذراً، حدث خطأ في جميع نماذج الذكاء الاصطناعي، يرجى المحاولة مرة أخرى لاحقاً.",
                    "citations": {}
                }
        ...
        # Try to get exact counts using the model's built-in tokenizer method
        try:
            lc_client = current_llm_model.get_client()
            if hasattr(lc_client, "get_num_tokens"):
                output_tokens = lc_client.get_num_tokens(full_generated_text)
        except Exception:
            pass
            
        # Override input tokens with usage_metadata if available (Gemini usually reports input correctly)
